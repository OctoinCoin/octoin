...
QString dateTimeStrDate(const QDateTime &date)
{
    return date.date().toString(Qt::SystemLocaleShortDate);
}

QString dateTimeStrTime(const QDateTime &date)
{
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a OctoinCoin address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("octoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("octoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "octoin:");
    }
...
{
    QString ret = QString("octoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "octoin.desktop";
    return GetAutostartDir() / strprintf("octoin-%s.lnk", chain);
}
