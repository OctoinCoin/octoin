...
class ReceivePaymentController(val handlers: Handlers, val stage: Stage) extends Logging {
  ...
  @FXML def handleGenerate(event: ActionEvent) = {
    if ((("milliOCC".equals(unit.getValue) || "Satoshi".equals(unit.getValue) || "OCC".equals(unit.getValue))
      ...
      try {
        ...
        val smartAmount = unit.getValue match {
          case "OCC" => Satoshi(parsedInt.toLong * 100000000L + amountDec.toLong * 100000L)
          case "milliOCC" => Satoshi(parsedInt.toLong * 100000L + amountDec.toLong * 100L)
        ...
        if (GUIValidators.validate(amountError, "Amount must be greater than 0", smartAmount.amount > 0)
          && GUIValidators.validate(amountError, f"Amount must be less than ${PaymentRequest.maxAmount.amount}%,d sat (~${PaymentRequest.maxAmount.amount / 100000000D}%.3f OCC)", smartAmount <= PaymentRequest.maxAmount)
          && GUIValidators.validate(amountError, f"Amount must be more than ${PaymentRequest.minAmount.amount}%,d sat (~${PaymentRequest.minAmount.amount / 100000000D}%.3f OCC)", smartAmount >= PaymentRequest.minAmount)
