...
class FxApp extends Application with Logging {
  ...
  def onError(t: Throwable): Unit = t match {
    ...
    case BitcoinRPCConnectionException =>
      notifyPreloader(new ErrorNotification("Setup", "Could not connect to OctoinCoin Core using JSON-RPC.", null))
      notifyPreloader(new AppNotification(InfoAppNotification, "Make sure that OctoinCoin Core is up and running and RPC parameters are correct."))
    case BitcoinZMQConnectionTimeoutException =>
      notifyPreloader(new ErrorNotification("Setup", "Could not connect to OctoinCoin Core using ZMQ.", null))
      notifyPreloader(new AppNotification(InfoAppNotification, "Make sure that OctoinCoin Core is up and running and ZMQ parameters are correct."))
