...
UniValue checkkernel(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "OctoinCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "OctoinCoin is downloading blocks...");
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "OctoinCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "OctoinCoin is downloading blocks...");
